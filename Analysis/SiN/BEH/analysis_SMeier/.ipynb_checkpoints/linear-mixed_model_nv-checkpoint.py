...
for train_index, test_index in kf.split(df_nv):
    X_train, X_test = df_nv.loc[train_index, ['levels', 'position', 'block']], df_nv.loc[test_index, ['levels', 'position', 'block']]
    y_train, y_test = df_nv.loc[train_index, 'accu'], df_nv.loc[test_index, 'accu']

    md_nv = sm.MixedLM(endog=y_train, exog=X_train, groups=df_nv.loc[train_index, 'subj'])
    mdf_nv = md_nv.fit()

    # Forecasting on test data
    y_pred = mdf_nv.predict(exog=X_test)
