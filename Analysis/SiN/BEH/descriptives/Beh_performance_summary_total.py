...
dfSNL_LF= pd.melt(dfSNL, id_vars=['subjID','noise','block','levels'], value_vars=['callSignCorrect', 'colourCorrect','numberCorrect'])
...
label = list()
for i in dfSNL_LF.index:
    tmp = dfSNL_LF['noise'][i],dfSNL_LF['levels'][i],dfSNL_LF['variable'][i]
    label.append( '_'.join(tmp))
dfSNL_LF.insert(5,'trial',label)    

dfWF = pd.pivot_table(dfSNL_LF,index='subjID',columns='trial',values='value')


#%% Save excel
if save == 1:
    df.to_csv(diroutput+ '\\'+taskID+'_beh_summary_long.csv')
    dfLF.to_csv(diroutput+ '\\'+taskID+'_beh_summary_looong.csv')
