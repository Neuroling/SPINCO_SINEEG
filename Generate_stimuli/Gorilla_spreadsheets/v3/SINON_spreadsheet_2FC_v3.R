...
# directories 
dirinput <- 'V:/spinco_data/LIRI_database/SINON_MATCH_v3/2FC_14items/'
databasefile <-'V:/spinco_data/LIRI_database/LIRI_database_stimuli.xlsx'
zoe <- openxlsx::read.xlsx('V:/spinco_data/LIRI_database/LIRI_database_subsets/liri_2FC_postselection_Zoe.xlsx') # table with neighbors 
...
for (i in 1:length(refTab$subset)){
  fileinput <- paste0('list2match14_set',i,'.txt')
  ...
  namesTab$item_distractor <- zoe$new_distractor[Filter(function(x) !is.na(x), match(namesTab$item_target,zoe$target))]
...
allNamesTab$block <- rep(c(1,2,3,4),each=70)
