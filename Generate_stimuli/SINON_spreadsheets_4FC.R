...
library(LexOPS)
library(stringr)
 #  CALL PACKAGE LEXOPS to Generate alternative items 
# --------------------------------------------------------
#
setwd("V:/spinco_data/LIRI_database/")
############################################################
dirinput <- 'V:/spinco_data/LIRI_database/SINON_LEXOPS_genMatched'
diroutput <- dirinput
mydatabase <- 'V:/spinco_data/LIRI_database/LIRI_database_stimuli.xlsx'
dirmatched <- 'V:/spinco_data/LIRI_database/SINON_MATCH_subsets_100words'
#----------------------------------------------------------------------------
...
liri <- select(mytab, c('urword','Pseudoword','PTAW','OTAW','lgSUBTLEX','nSyllables','PTAN'))
liri$Pseudoword <- gsub('-','',liri$Pseudoword)

liri$altP <- liri$PSAW
liri$altO <- liri$OSAW
for (i in 1:nrow(liri)) {
  tmp <- strsplit(liri$PTAW[i],';','')   
  liri$altPhono[i] <- paste0(tmp[[1]][which(sapply(tmp,str_detect,"[[:upper:]]")==TRUE)],collapse=';')
  liri$altPhono1[i] <- tmp[[1]][which(sapply(tmp,str_detect,"[[:upper:]]")==TRUE)][1]
  
  tmp <- strsplit(liri$OTAW[i],';','')  
  liri$altOrtho[i] <- paste0(tmp[[1]][which(sapply(tmp,str_detect,"[[:upper:]]")==TRUE)],collapse=';')
}
## get now additional pseudowords from our wuggy
wuggy <- openxlsx::read.xlsx(mydatabase,sheet = 'Wuggy')
wuggy$Word <- gsub('-','',wuggy$Word)
wuggy$Match  <- gsub('-','',wuggy$Match)
wuggy <- wuggy[which(wuggy$Word %in% liri$urword),]

liri$Pseudoword2 <- liri$Pseudoword
for (i in 1:length(liri$urword)){
  tmpWuggy <- wuggy[which(wuggy$Word == liri$urword[i]),]
  tmpWuggy <- tmpWuggy[-which(tmpWuggy$Match == liri$Pseudoword[i]),]
  tmpWuggy <- tmpWuggy[order(abs(tmpWuggy$Ned1_Diff)),]
  liri$Pseudoword2_alts[i] <- paste0(tmpWuggy$Match, collapse = ';')
  liri$Pseudoword2[i] <- tmpWuggy$Match[1]
}
openxlsx::write.xlsx(liri,'liri_tmp_distractors4FC.xlsx')
