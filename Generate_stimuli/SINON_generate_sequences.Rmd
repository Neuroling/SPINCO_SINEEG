...
#zoom: 2
  ...
  [Reject outliers ] n=325-->[<comment> Sets of n trials |Set1 | Set2| Set3| Set4| Set4] 
  [<comment> Sets of n trials |Set1 | Set2| Set3| Set4| Set4]-- [<usecase>pseudorandomization*]
  [<usecase>pseudorandomization*]-- [<usecase>Check balanced features]
  [<usecase>Check balanced features]-:> [Experiment Blocks]
  

```

Read data, get boxplot statistics and remove outliers
...
Divide into sets, while checking they don't differ in key features
...
## Contingency tables and compare proportions
  ...
  if (stat$chisq$p.value < 0.05){
    stop('number of item length is unbalance between the sets!')
  }  
  # syllable count 
  stat <- dlookr::compare_category(setIdx, syllableCount,.data = dat) %>%  summary()
  if (stat$chisq$p.value < 0.05){
    stop('number of syllables is unbalance between the sets!')
  }
...
for (i  in 1:length(sets)) {
     ...
     colnames(Blcks) <- paste0('seq',1:dim(Blcks)[2])
...
### Sets  {.tabset .tabset-pills .tabset-fade}
