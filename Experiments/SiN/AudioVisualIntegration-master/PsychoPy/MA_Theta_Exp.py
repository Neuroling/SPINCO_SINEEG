...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='V:\\gfraga\\scripts_neulin\\Projects\\SINEEG\\Experiments\\SiN\\AudioVisualIntegration-master\\PsychoPy\\MA_Theta_Exp.py',
...
originalSentence = sound.Sound('A', secs=-1, stereo=True, hamming=True,
    name='originalSentence')
originalSentence.setVolume(1)
waitAfterSound = visual.ImageStim(
    win=win,
    name='waitAfterSound', 
...
shortPauseText = visual.TextStim(win=win, name='shortPauseText',
    text='Kurze Pause.\n\nNach dem Signalton geht es gleich weiter.',
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=0.0);
shortPauseTone = sound.Sound('tone.wav', secs=-1, stereo=True, hamming=True,
    name='shortPauseTone')
shortPauseTone.setVolume(1)
waitAfterTone = visual.ImageStim(
    win=win,
    name='waitAfterTone', 
    image='images/blankScreen.png', mask=None, anchor='center',
    ori=0, pos=(0, 0), size=[1920, 1080],
...
longPauseText = visual.TextStim(win=win, name='longPauseText',
    text='Du hast nun die Hälfte geschafft!\n\nDu kannst dir gerne eine Pause nehmen.\n\nDrücke bitte die Leertaste, wenn du bereit bist fortzufahren.',
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=0.0);
longPauseResponse = keyboard.Keyboard()
...
exp = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('exposureStimuliList.xlsx'),
    seed=None, name='exp')
thisExp.addLoop(exp)  # add the loop to the experiment
thisExp_2 = exp.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisExp_2.rgb)
if thisExp_2 != None:
    for paramName in thisExp_2:
        exec('{} = thisExp_2[paramName]'.format(paramName))

for thisExp_2 in exp:
    currentLoop = exp
    # abbreviate parameter names if possible (e.g. rgb = thisExp_2.rgb)
    if thisExp_2 != None:
        for paramName in thisExp_2:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    originalSentence.setSound(audioFile, hamming=True)
    originalSentence.setVolume(1, log=False)
    # keep track of which components have finished
    exposureComponents = [originalSentence, waitAfterSound]
    ...
    while continueRoutine:
        ...
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        # start/stop originalSentence
        if originalSentence.status == NOT_STARTED and frameN >= 0.0:
            # keep track of start time/frame for later
            originalSentence.frameNStart = frameN  # exact frame index
            originalSentence.tStart = t  # local t and not account for scr refresh
            originalSentence.tStartRefresh = tThisFlipGlobal  # on global time
            # add timestamp to datafile
            thisExp.addData('originalSentence.started', t)
            originalSentence.play()  # start the sound (it finishes automatically)
        
        # *waitAfterSound* updates
        if waitAfterSound.status == NOT_STARTED and originalSentence.status==FINISHED:
            # keep track of start time/frame for later
            waitAfterSound.frameNStart = frameN  # exact frame index
            waitAfterSound.tStart = t  # local t and not account for scr refresh
            waitAfterSound.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(waitAfterSound, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'waitAfterSound.started')
            waitAfterSound.setAutoDraw(True)
        if waitAfterSound.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > waitAfterSound.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
                waitAfterSound.tStop = t  # not accounting for scr refresh
                waitAfterSound.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'waitAfterSound.stopped')
                waitAfterSound.setAutoDraw(False)
    ...
    for thisComponent in exposureComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    originalSentence.stop()  # ensure sound has stopped at end of routine
    # the Routine "exposure" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
# completed 1 repeats of 'exp'
...
training = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('trainingStimuliList.xlsx'),
...
# set up handler to look after randomisation of conditions etc
firstHalf = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(firstHalf),
    seed=None, name='firstHalf')
thisExp.addLoop(firstHalf)  # add the loop to the experiment
thisFirstHalf = firstHalf.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisFirstHalf.rgb)
if thisFirstHalf != None:
    for paramName in thisFirstHalf:
        exec('{} = thisFirstHalf[paramName]'.format(paramName))

for thisFirstHalf in firstHalf:
    currentLoop = firstHalf
    # abbreviate parameter names if possible (e.g. rgb = thisFirstHalf.rgb)
    if thisFirstHalf != None:
        for paramName in thisFirstHalf:
            exec('{} = thisFirstHalf[paramName]'.format(paramName))
    ...
    # set up handler to look after randomisation of conditions etc
    blocksA = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(block),
        seed=None, name='blocksA')
    thisExp.addLoop(blocksA)  # add the loop to the experiment
    thisBlocksA = blocksA.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisBlocksA.rgb)
    if thisBlocksA != None:
        for paramName in thisBlocksA:
            exec('{} = thisBlocksA[paramName]'.format(paramName))
    
    for thisBlocksA in blocksA:
        currentLoop = blocksA
        # abbreviate parameter names if possible (e.g. rgb = thisBlocksA.rgb)
        if thisBlocksA != None:
            for paramName in thisBlocksA:
        ...
        for thisComponent in trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store data for blocksA (TrialHandler)
        blocksA.addData('mouseClickOnCall.x', mouseClickOnCall.x)
        blocksA.addData('mouseClickOnCall.y', mouseClickOnCall.y)
        blocksA.addData('mouseClickOnCall.leftButton', mouseClickOnCall.leftButton)
        blocksA.addData('mouseClickOnCall.midButton', mouseClickOnCall.midButton)
        blocksA.addData('mouseClickOnCall.rightButton', mouseClickOnCall.rightButton)
        blocksA.addData('mouseClickOnCall.time', mouseClickOnCall.time)
        blocksA.addData('mouseClickOnCall.clicked_name', mouseClickOnCall.clicked_name)
        # store data for blocksA (TrialHandler)
        blocksA.addData('mouseClickOnColour.x', mouseClickOnColour.x)
        blocksA.addData('mouseClickOnColour.y', mouseClickOnColour.y)
        blocksA.addData('mouseClickOnColour.leftButton', mouseClickOnColour.leftButton)
        blocksA.addData('mouseClickOnColour.midButton', mouseClickOnColour.midButton)
        blocksA.addData('mouseClickOnColour.rightButton', mouseClickOnColour.rightButton)
        blocksA.addData('mouseClickOnColour.time', mouseClickOnColour.time)
        blocksA.addData('mouseClickOnColour.clicked_name', mouseClickOnColour.clicked_name)
        # store data for blocksA (TrialHandler)
        blocksA.addData('mouseClickOnNumber.x', mouseClickOnNumber.x)
        blocksA.addData('mouseClickOnNumber.y', mouseClickOnNumber.y)
        blocksA.addData('mouseClickOnNumber.leftButton', mouseClickOnNumber.leftButton)
        blocksA.addData('mouseClickOnNumber.midButton', mouseClickOnNumber.midButton)
        blocksA.addData('mouseClickOnNumber.rightButton', mouseClickOnNumber.rightButton)
        blocksA.addData('mouseClickOnNumber.time', mouseClickOnNumber.time)
        blocksA.addData('mouseClickOnNumber.clicked_name', mouseClickOnNumber.clicked_name)
        # Run 'End Routine' code from evaluateResponses
        #evaluate correctness of response and write into a new column TRUE, FALSE, or NO_ANSW  
        if len(mouseClickOnCall.clicked_name) > 0:
            currentLoop.addData('callSignCorrect', mapCallSign[mouseClickOnCall.clicked_name[0]] == callSign) 
    ...
    # completed 1 repeats of 'blocksA'
    
    
    # --- Prepare to start Routine "short_pause" ---
    continueRoutine = True
    # update component parameters for each repeat
    shortPauseTone.setSound('tone.wav', hamming=True)
    shortPauseTone.setVolume(1, log=False)
    # Run 'Begin Routine' code from saveAfterEachBlock
    #sicherIstSicher SAve after each block
    thisExp.saveAsWideText(filename+'.csv')
    logging.flush()
    # keep track of which components have finished
    short_pauseComponents = [shortPauseText, shortPauseTone, waitAfterTone]
    ...
    while continueRoutine:
        ...
        # *shortPauseText* updates
        if shortPauseText.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
            shortPauseText.frameNStart = frameN  # exact frame index
            shortPauseText.tStart = t  # local t and not account for scr refresh
            shortPauseText.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(shortPauseText, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'shortPauseText.started')
            shortPauseText.setAutoDraw(True)
        if shortPauseText.status == STARTED:
            # is it time to stop? (based on local clock)
            if tThisFlip > 9.5-frameTolerance:
                # keep track of stop time/frame for later
                shortPauseText.tStop = t  # not accounting for scr refresh
                shortPauseText.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'shortPauseText.stopped')
                shortPauseText.setAutoDraw(False)
        # Run 'Each Frame' code from skipShortPause
        if seq == 4:
            continueRoutine = False
        # start/stop shortPauseTone
        if shortPauseTone.status == NOT_STARTED and shortPauseText.status==FINISHED:
            # keep track of start time/frame for later
            shortPauseTone.frameNStart = frameN  # exact frame index
            shortPauseTone.tStart = t  # local t and not account for scr refresh
            shortPauseTone.tStartRefresh = tThisFlipGlobal  # on global time
            # add timestamp to datafile
            thisExp.addData('shortPauseTone.started', tThisFlipGlobal)
            shortPauseTone.play(when=win)  # sync with win flip
        
        # *waitAfterTone* updates
        if waitAfterTone.status == NOT_STARTED and shortPauseTone.status==FINISHED:
            # keep track of start time/frame for later
            waitAfterTone.frameNStart = frameN  # exact frame index
            waitAfterTone.tStart = t  # local t and not account for scr refresh
            waitAfterTone.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(waitAfterTone, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'waitAfterTone.started')
            waitAfterTone.setAutoDraw(True)
        if waitAfterTone.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > waitAfterTone.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                waitAfterTone.tStop = t  # not accounting for scr refresh
                waitAfterTone.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'waitAfterTone.stopped')
                waitAfterTone.setAutoDraw(False)
    ...
    for thisComponent in short_pauseComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    shortPauseTone.stop()  # ensure sound has stopped at end of routine
    # the Routine "short_pause" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
# completed 1 repeats of 'firstHalf'


# --- Prepare to start Routine "long_pause" ---
continueRoutine = True
# update component parameters for each repeat
longPauseResponse.keys = []
longPauseResponse.rt = []
_longPauseResponse_allKeys = []
# keep track of which components have finished
long_pauseComponents = [longPauseText, longPauseResponse]
...
while continueRoutine:
    ...
    # *longPauseText* updates
    if longPauseText.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        # keep track of start time/frame for later
        longPauseText.frameNStart = frameN  # exact frame index
        longPauseText.tStart = t  # local t and not account for scr refresh
        longPauseText.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(longPauseText, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'longPauseText.started')
        longPauseText.setAutoDraw(True)
    
    # *longPauseResponse* updates
    waitOnFlip = False
    if longPauseResponse.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        longPauseResponse.frameNStart = frameN  # exact frame index
        longPauseResponse.tStart = t  # local t and not account for scr refresh
        longPauseResponse.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(longPauseResponse, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'longPauseResponse.started')
        longPauseResponse.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(longPauseResponse.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(longPauseResponse.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if longPauseResponse.status == STARTED and not waitOnFlip:
        theseKeys = longPauseResponse.getKeys(keyList=['space'], waitRelease=False)
        _longPauseResponse_allKeys.extend(theseKeys)
        if len(_longPauseResponse_allKeys):
            longPauseResponse.keys = _longPauseResponse_allKeys[-1].name  # just the last key pressed
            longPauseResponse.rt = _longPauseResponse_allKeys[-1].rt
            # a response ends the routine
            continueRoutine = False
...
# set up handler to look after randomisation of conditions etc
secondHalf = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(secondHalf),
    seed=None, name='secondHalf')
thisExp.addLoop(secondHalf)  # add the loop to the experiment
thisSecondHalf = secondHalf.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisSecondHalf.rgb)
if thisSecondHalf != None:
    for paramName in thisSecondHalf:
        exec('{} = thisSecondHalf[paramName]'.format(paramName))

for thisSecondHalf in secondHalf:
    currentLoop = secondHalf
    # abbreviate parameter names if possible (e.g. rgb = thisSecondHalf.rgb)
    if thisSecondHalf != None:
        for paramName in thisSecondHalf:
            exec('{} = thisSecondHalf[paramName]'.format(paramName))
    ...
    # set up handler to look after randomisation of conditions etc
    blocksB = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(block),
        seed=None, name='blocksB')
    thisExp.addLoop(blocksB)  # add the loop to the experiment
    thisBlocksB = blocksB.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisBlocksB.rgb)
    if thisBlocksB != None:
        for paramName in thisBlocksB:
            exec('{} = thisBlocksB[paramName]'.format(paramName))
    
    for thisBlocksB in blocksB:
        currentLoop = blocksB
        # abbreviate parameter names if possible (e.g. rgb = thisBlocksB.rgb)
        if thisBlocksB != None:
            for paramName in thisBlocksB:
        ...
        for thisComponent in trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store data for blocksB (TrialHandler)
        blocksB.addData('mouseClickOnCall.x', mouseClickOnCall.x)
        blocksB.addData('mouseClickOnCall.y', mouseClickOnCall.y)
        blocksB.addData('mouseClickOnCall.leftButton', mouseClickOnCall.leftButton)
        blocksB.addData('mouseClickOnCall.midButton', mouseClickOnCall.midButton)
        blocksB.addData('mouseClickOnCall.rightButton', mouseClickOnCall.rightButton)
        blocksB.addData('mouseClickOnCall.time', mouseClickOnCall.time)
        blocksB.addData('mouseClickOnCall.clicked_name', mouseClickOnCall.clicked_name)
        # store data for blocksB (TrialHandler)
        blocksB.addData('mouseClickOnColour.x', mouseClickOnColour.x)
        blocksB.addData('mouseClickOnColour.y', mouseClickOnColour.y)
        blocksB.addData('mouseClickOnColour.leftButton', mouseClickOnColour.leftButton)
        blocksB.addData('mouseClickOnColour.midButton', mouseClickOnColour.midButton)
        blocksB.addData('mouseClickOnColour.rightButton', mouseClickOnColour.rightButton)
        blocksB.addData('mouseClickOnColour.time', mouseClickOnColour.time)
        blocksB.addData('mouseClickOnColour.clicked_name', mouseClickOnColour.clicked_name)
        # store data for blocksB (TrialHandler)
        blocksB.addData('mouseClickOnNumber.x', mouseClickOnNumber.x)
        blocksB.addData('mouseClickOnNumber.y', mouseClickOnNumber.y)
        blocksB.addData('mouseClickOnNumber.leftButton', mouseClickOnNumber.leftButton)
        blocksB.addData('mouseClickOnNumber.midButton', mouseClickOnNumber.midButton)
        blocksB.addData('mouseClickOnNumber.rightButton', mouseClickOnNumber.rightButton)
        blocksB.addData('mouseClickOnNumber.time', mouseClickOnNumber.time)
        blocksB.addData('mouseClickOnNumber.clicked_name', mouseClickOnNumber.clicked_name)
        # Run 'End Routine' code from evaluateResponses
        #evaluate correctness of response and write into a new column TRUE, FALSE, or NO_ANSW  
        if len(mouseClickOnCall.clicked_name) > 0:
            currentLoop.addData('callSignCorrect', mapCallSign[mouseClickOnCall.clicked_name[0]] == callSign) 
    ...
    # completed 1 repeats of 'blocksB'
    ...
    # the Routine "short_pause" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
# completed 1 repeats of 'secondHalf'
