...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='Z:\\Projects\\Spinco\\SINEEG\\Scripts\\Experiments\\SiN\\SiN_task\\SIN_lab_27-06-23_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    thisExp.addData('instrStart.started', globalClock.getTime())
    ...
    thisExp.addData('instrStart.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('instrRest_PRE.started', globalClock.getTime())
    ...
    for thisComponent in instrRest_PREComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('instrRest_PRE.stopped', globalClock.getTime())
    if pp_instrRest_PRE.status == STARTED:
        win.callOnFlip(pp_instrRest_PRE.setData, int(0))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('restTrial_pre.started', globalClock.getTime())
    ...
    for thisComponent in restTrial_preComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('restTrial_pre.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('instrMain_1.started', globalClock.getTime())
    ...
    for thisComponent in instrMain_1Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('instrMain_1.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('instrMain_2.started', globalClock.getTime())
    ...
    for thisComponent in instrMain_2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('instrMain_2.stopped', globalClock.getTime())
    if pp_instr_start_2.status == STARTED:
        win.callOnFlip(pp_instr_start_2.setData, int(0))
    ...
    if thisTrials_eg != None:
        for paramName in thisTrials_eg:
            globals()[paramName] = thisTrials_eg[paramName]
    ...
    for thisTrials_eg in trials_eg:
        ...
        while continueRoutine:
            ...
            # if sound_examples is starting this frame...
            if sound_examples.status == NOT_STARTED and frameN >= instr_start_2.status == FINISHED:
                ...
                # update status
                sound_examples.status = STARTED
                sound_examples.play(when=win)  # sync with win flip
            # update sound_examples status according to whether it's playing
            if sound_examples.isPlaying:
                sound_examples.status = STARTED
            elif sound_examples.isFinished:
                sound_examples.status = FINISHED
            ...
            # if screenAfterAudio_2 is starting this frame...
            if screenAfterAudio_2.status == NOT_STARTED and frameN >= fixation_2.status==FINISHED  :
                ...
                # update status
                screenAfterAudio_2.status = STARTED
                screenAfterAudio_2.setAutoDraw(True)
            
            # if screenAfterAudio_2 is active this frame...
            if screenAfterAudio_2.status == STARTED:
                # update params
                pass
            
            # if screenAfterAudio_2 is stopping this frame...
            if screenAfterAudio_2.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 1-frameTolerance:
                    ...
                    # update status
                    screenAfterAudio_2.status = FINISHED
        ...
        for thisComponent in audioTrial_egComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('audioTrial_eg.stopped', globalClock.getTime())
        sound_examples.pause()  # ensure sound has stopped at end of Routine
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('trial_eg.started', globalClock.getTime())
        ...
        while continueRoutine:
            ...
            # if blankScreen_2 is starting this frame...
            ...
            # if blankScreen_2 is active this frame...
            if blankScreen_2.status == STARTED:
                # update params
                pass
            
            # if blankScreen_2 is stopping this frame...
            if blankScreen_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blankScreen_2.tStartRefresh + 1.0-frameTolerance:
                    ...
                    # update status
                    blankScreen_2.status = FINISHED
                    blankScreen_2.setAutoDraw(False)
            
            # *call1_2* updates
            
            # if call1_2 is starting this frame...
            if call1_2.status == NOT_STARTED and blankScreen_2.status==FINISHED:
                ...
                # update status
                call1_2.status = STARTED
                call1_2.setAutoDraw(True)
            
            # if call1_2 is active this frame...
            if call1_2.status == STARTED:
                # update params
                pass
            
            # if call1_2 is stopping this frame...
            if call1_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    ...
                    # update status
                    call1_2.status = FINISHED
                    call1_2.setAutoDraw(False)
            
            # *call2_2* updates
            
            # if call2_2 is starting this frame...
            if call2_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                call2_2.status = STARTED
                call2_2.setAutoDraw(True)
            
            # if call2_2 is active this frame...
            if call2_2.status == STARTED:
                # update params
                pass
            
            # if call2_2 is stopping this frame...
            if call2_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    call2_2.status = FINISHED
                    call2_2.setAutoDraw(False)
            
            # *call3_2* updates
            
            # if call3_2 is starting this frame...
            if call3_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                call3_2.status = STARTED
                call3_2.setAutoDraw(True)
            
            # if call3_2 is active this frame...
            if call3_2.status == STARTED:
                # update params
                pass
            
            # if call3_2 is stopping this frame...
            if call3_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    call3_2.status = FINISHED
                    call3_2.setAutoDraw(False)
            
            # *call4_2* updates
            
            # if call4_2 is starting this frame...
            if call4_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                call4_2.status = STARTED
                call4_2.setAutoDraw(True)
            
            # if call4_2 is active this frame...
            if call4_2.status == STARTED:
                # update params
                pass
            
            # if call4_2 is stopping this frame...
            if call4_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    call4_2.status = FINISHED
                    call4_2.setAutoDraw(False)
            
            # *colour1_2* updates
            
            # if colour1_2 is starting this frame...
            if colour1_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                colour1_2.status = STARTED
                colour1_2.setAutoDraw(True)
            
            # if colour1_2 is active this frame...
            if colour1_2.status == STARTED:
                # update params
                pass
            
            # if colour1_2 is stopping this frame...
            if colour1_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    colour1_2.status = FINISHED
                    colour1_2.setAutoDraw(False)
            
            # *colour2_2* updates
            
            # if colour2_2 is starting this frame...
            if colour2_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                colour2_2.status = STARTED
                colour2_2.setAutoDraw(True)
            
            # if colour2_2 is active this frame...
            if colour2_2.status == STARTED:
                # update params
                pass
            
            # if colour2_2 is stopping this frame...
            if colour2_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    colour2_2.status = FINISHED
                    colour2_2.setAutoDraw(False)
            
            # *colour3_2* updates
            
            # if colour3_2 is starting this frame...
            if colour3_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                colour3_2.status = STARTED
                colour3_2.setAutoDraw(True)
            
            # if colour3_2 is active this frame...
            if colour3_2.status == STARTED:
                # update params
                pass
            
            # if colour3_2 is stopping this frame...
            if colour3_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    colour3_2.status = FINISHED
                    colour3_2.setAutoDraw(False)
            
            # *colour4_2* updates
            
            # if colour4_2 is starting this frame...
            if colour4_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                colour4_2.status = STARTED
                colour4_2.setAutoDraw(True)
            
            # if colour4_2 is active this frame...
            if colour4_2.status == STARTED:
                # update params
                pass
            
            # if colour4_2 is stopping this frame...
            if colour4_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    colour4_2.status = FINISHED
                    colour4_2.setAutoDraw(False)
            
            # *number1_2* updates
            
            # if number1_2 is starting this frame...
            if number1_2.status == NOT_STARTED and blankScreen_2.status==FINISHED:
                ...
                # update status
                number1_2.status = STARTED
                number1_2.setAutoDraw(True)
            
            # if number1_2 is active this frame...
            if number1_2.status == STARTED:
                # update params
                pass
            
            # if number1_2 is stopping this frame...
            if number1_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    number1_2.status = FINISHED
                    number1_2.setAutoDraw(False)
            
            # *number2_2* updates
            
            # if number2_2 is starting this frame...
            if number2_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                number2_2.status = STARTED
                number2_2.setAutoDraw(True)
            
            # if number2_2 is active this frame...
            if number2_2.status == STARTED:
                # update params
                pass
            
            # if number2_2 is stopping this frame...
            if number2_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    number2_2.status = FINISHED
                    number2_2.setAutoDraw(False)
            
            # *number3_2* updates
            
            # if number3_2 is starting this frame...
            if number3_2.status == NOT_STARTED and blankScreen_2.status==FINISHED:
                ...
                # update status
                number3_2.status = STARTED
                number3_2.setAutoDraw(True)
            
            # if number3_2 is active this frame...
            if number3_2.status == STARTED:
                # update params
                pass
            
            # if number3_2 is stopping this frame...
            if number3_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    number3_2.status = FINISHED
                    number3_2.setAutoDraw(False)
            
            # *number4_2* updates
            
            # if number4_2 is starting this frame...
            if number4_2.status == NOT_STARTED and blankScreen_2.status ==FINISHED:
                ...
                # update status
                number4_2.status = STARTED
                number4_2.setAutoDraw(True)
            
            # if number4_2 is active this frame...
            if number4_2.status == STARTED:
                # update params
                pass
            
            # if number4_2 is stopping this frame...
            if number4_2.status == STARTED:
                if bool(blankScreenAfterResponse_2.status == STARTED):
                    # keep track of stop time/frame for later
                    ...
                    # update status
                    number4_2.status = FINISHED
                    number4_2.setAutoDraw(False)
            # *mouseClickOnCall_2* updates
            
            # if mouseClickOnCall_2 is starting this frame...
            ...
            if mouseClickOnCall_2.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames([call1_2, call2_2, call3_2, call4_2], namespace=locals())
            ...
            # if mouseClickOnColour_2 is starting this frame...
            ...
            if mouseClickOnColour_2.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames([colour1_2, colour2_2, colour3_2, colour4_2], namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
            ...
            # if mouseClickOnNumber_2 is starting this frame...
            ...
            if mouseClickOnNumber_2.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames([number1_2, number2_2, number3_2, number4_2], namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
            ...
            # if blankScreenAfterResponse_2 is starting this frame...
            if blankScreenAfterResponse_2.status == NOT_STARTED and mouseClickOnNumber.status==FINISHED:
                ...
                # update status
                blankScreenAfterResponse_2.status = STARTED
                blankScreenAfterResponse_2.setAutoDraw(True)
            
            # if blankScreenAfterResponse_2 is active this frame...
            if blankScreenAfterResponse_2.status == STARTED:
                # update params
                pass
            
            # if blankScreenAfterResponse_2 is stopping this frame...
            if blankScreenAfterResponse_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blankScreenAfterResponse_2.tStartRefresh + 0.3-frameTolerance:
                    ...
                    # update status
                    blankScreenAfterResponse_2.status = FINISHED
        ...
        for thisComponent in trial_egComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('trial_eg.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('instrTask.started', globalClock.getTime())
    key_resp_inst.keys = []
    key_resp_inst.rt = []
    ...
    for thisComponent in instrTaskComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('instrTask.stopped', globalClock.getTime())
    if pp_instrTask.status == STARTED:
        win.callOnFlip(pp_instrTask.setData, int(0))
    ...
    for thisBlock in blocks:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('startBlock.started', globalClock.getTime())
        key_resp.keys = []
        key_resp.rt = []
        _key_resp_allKeys = []
        # keep track of which components have finished
        ...
        for thisComponent in startBlockComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('startBlock.stopped', globalClock.getTime())
        if pp_blockstart.status == STARTED:
            win.callOnFlip(pp_blockstart.setData, int(0))
        ...
        for thisTrial in trials:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('audioTrial.started', globalClock.getTime())
            ...
            for thisComponent in audioTrialComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('audioTrial.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('instrRest_POST.started', globalClock.getTime())
    ...
    for thisComponent in instrRest_POSTComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('instrRest_POST.stopped', globalClock.getTime())
    if pp_rest_post.status == STARTED:
        win.callOnFlip(pp_rest_post.setData, int(0))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('restTrial_post.started', globalClock.getTime())
    ...
    for thisComponent in restTrial_postComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('restTrial_post.stopped', globalClock.getTime())
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('thanks_end.started', globalClock.getTime())
    key_resp_6.keys = []
    key_resp_6.rt = []
    _key_resp_6_allKeys = []
    ...
    for thisComponent in thanks_endComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('thanks_end.stopped', globalClock.getTime())
