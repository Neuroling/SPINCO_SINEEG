...
for thisBlock in blocks:
    ...
    for thisTrial in trials:
        ...
        # keep track of which components have finished
        trialComponents = [blankScreen, call1, call2, call3, call4, colour1, colour2, colour3, colour4, number1, number2, number3, number4, mouseClickOnCall, pp_mouse_call, mouseClickOnColour, pp_mouse_colour, mouseClickOnNumber, pp_mouse_number, blankScreenAfterResponse, pp_respGrid]
        ...
        while continueRoutine:
            ...
            # Run 'Each Frame' code from disableCallButtons
                        if gotValidClick:
                            ...
                            continueRoutine = True
            # *pp_mouse_call* updates
            if pp_mouse_call.status == NOT_STARTED and mouseClickOnCall.getPressed()[0]:
                # keep track of start time/frame for later
                pp_mouse_call.frameNStart = frameN  # exact frame index
                pp_mouse_call.tStart = t  # local t and not account for scr refresh
                pp_mouse_call.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(pp_mouse_call, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'pp_mouse_call.started')
                pp_mouse_call.status = STARTED
                win.callOnFlip(pp_mouse_call.setData, int(1))
            if pp_mouse_call.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > pp_mouse_call.tStartRefresh + 0.01-frameTolerance:
                    # keep track of stop time/frame for later
                    pp_mouse_call.tStop = t  # not accounting for scr refresh
                    pp_mouse_call.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'pp_mouse_call.stopped')
                    pp_mouse_call.status = FINISHED
                    win.callOnFlip(pp_mouse_call.setData, int(0))
            ...
            # Run 'Each Frame' code from disableColourButtons
                        if gotValidClick:
                            ...
                            continueRoutine = True
            # *pp_mouse_colour* updates
            if pp_mouse_colour.status == NOT_STARTED and mouseClickOnColour.status == FINISHED:
                # keep track of start time/frame for later
                pp_mouse_colour.frameNStart = frameN  # exact frame index
                pp_mouse_colour.tStart = t  # local t and not account for scr refresh
                pp_mouse_colour.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(pp_mouse_colour, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'pp_mouse_colour.started')
                pp_mouse_colour.status = STARTED
                win.callOnFlip(pp_mouse_colour.setData, int(2))
            if pp_mouse_colour.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > pp_mouse_colour.tStartRefresh + 0.01-frameTolerance:
                    # keep track of stop time/frame for later
                    pp_mouse_colour.tStop = t  # not accounting for scr refresh
                    pp_mouse_colour.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'pp_mouse_colour.stopped')
                    pp_mouse_colour.status = FINISHED
                    win.callOnFlip(pp_mouse_colour.setData, int(0))
            # *mouseClickOnNumber* updates
            if mouseClickOnNumber.status == NOT_STARTED and mouseClickOnCall.status == FINISHED:
                # keep track of start time/frame for later
            ...
            # Run 'Each Frame' code from disableNumberButtons
                        if gotValidClick:
                          ...
                          #  continueRoutine = True
            # *pp_mouse_number* updates
            if pp_mouse_number.status == NOT_STARTED and mouseClickOnNumber.status==FINISHEDD:
                # keep track of start time/frame for later
                pp_mouse_number.frameNStart = frameN  # exact frame index
                pp_mouse_number.tStart = t  # local t and not account for scr refresh
                pp_mouse_number.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(pp_mouse_number, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'pp_mouse_number.started')
                pp_mouse_number.status = STARTED
                win.callOnFlip(pp_mouse_number.setData, int(3))
            if pp_mouse_number.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > pp_mouse_number.tStartRefresh + 0.01-frameTolerance:
                    # keep track of stop time/frame for later
                    pp_mouse_number.tStop = t  # not accounting for scr refresh
                    pp_mouse_number.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'pp_mouse_number.stopped')
                    pp_mouse_number.status = FINISHED
                    win.callOnFlip(pp_mouse_number.setData, int(0))
