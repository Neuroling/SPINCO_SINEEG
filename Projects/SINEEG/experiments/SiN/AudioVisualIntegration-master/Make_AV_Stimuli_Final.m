...
%  Pulses=Noise.*Cycles;
%  OutFile='TestPulse_final.avi';
%  %Resample Cycles to fps
%  Window=[zeros(1,SilDur*fs_video),ones(1,BurstDur*fs_video)];
%  Cycles=[repmat(Window,1,Nwindows)];
%  visualise(Pulses,Cycles,fs,[],OutFile,'radius','gray',[],0,1,path2ffmpeg,fs_video,15)
 %end
...
LAGinSec=[0.093];%0.077;%positive values mean push the audio back in time (i.e. start later) relative to the video (calibration placed lag at -77ms)
...
if ~isempty(LAGinSec);
    for SoundFile=SoundFiles
        ...
        try
            ...
            BPThetaBursts=0.5*BPThetaBursts;
        ...
        if VISUALISE
            ...
            for VisType=SELECTEDVISUALISATIONS
                ...
                try
                    ...
                    %Global Theta needs audio to be lagged
                    %OutFile=fullfile(OutPath,[filename,'_NoThetaInAudio_GlobalThetaInVideo' Vprfx, '.avi']);
                    OutFile=fullfile(OutPath,[filename,'_NoThetaInAudio_GlobalThetaInVideo', '.avi']);

                    OutFileCtrl=fullfile(OutPath,[filename,'_NoThetaInAudio_NoVideo.avi']);
                    ...
                    %REMEMBER: NoTheta here is already lagged!
                    visualise(NoTheta,ThetaVideoEnvs,fs,centerfreqs,OutFile,VisType{:},'gray',[],0,CircleSize,path2ffmpeg,fs_video,blurAmount);
                    %delete uncompressed video:
                    %delete(OutFile);
                    
                    % control condition
                    visualise(NoTheta,MinVal*ones(size(ThetaVideoEnvs)),fs,centerfreqs,OutFileCtrl,...
                        VisType{:},'gray',[],0,StaticCircleSize,path2ffmpeg,fs_video,blurAmountFix);
                    %delete uncompressed video:
                    %delete(OutFileCtrl);
                    
                    
                    % Visualise Noise Vocoded Audio using GlobalTheta as visual input;
                    %NB these vocoded stimuli with 10Hz LP envelope filteri
                    %ng are Ghitza-controls
                    OutFile=fullfile(OutPath,[filename,'_NVocInAudio_GlobalThetaInVideo' '.avi']);
                    %OutFile=fullfile(OutPath,[filename,'_NVocInAudio_GlobalThetaInVideo' Vprfx '.avi']);
                    OutFileCtrl=fullfile(OutPath,[filename,'_NVocInAudio_NoVideo.avi']);
                    
                    % experimental condition: theta in video + audio
                    visualise(LaggedNV,ThetaVideoEnvs,fs,centerfreqs,OutFile,VisType{:},'gray',[],0,...
                        CircleSize,path2ffmpeg,fs_video,blurAmount);
                    %delete uncompressed video:
                    %delete(OutFile);
                    % control condition: audio with black screen
                    visualise(LaggedNV,MinVal*ones(size(ThetaVideoEnvs)),fs,centerfreqs,OutFileCtrl,VisType{:},'gray',[],0,...
                        StaticCircleSize,path2ffmpeg,fs_video,blurAmountFix);
                    %delete uncompressed video:
                    %delete(OutFileCtrl);
                    
                    
                    % write two more control files:
                    % NVGLobalTheta and Globaltheta
                    %OutFileCtrl=fullfile(OutPath,[filename,'_NVocGlobalTheta_NoVideo.avi']);
                    %visualise(NVGlobalTheta',MinVal*ones(size(GlobalTheta)),fs,centerfreqs,OutFileCtrl,VisType{:},'gray',[],1,...
                    %    StaticCircleSize,path2ffmpeg,fs_video);
                    %delete uncompressed video:
                    %delete(OutFileCtrl);
