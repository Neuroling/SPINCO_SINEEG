...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Becker\\Documents\\Neurofeedback\\Acquisition Material\\3_Revised_Acquisition_Material_RevDesign_3\\Acquisition Material\\PsychoPy\\triggered_experiment_revDesign_lastrun.py',
...
if use_debug:
    nTrials_1Block = 4 # number of trials in each block, normal is 70
...
Instruction_CalibrationClock = core.Clock()
...
degrad = [*np.zeros(nTrials_1Block), *np.zeros(nTrials_1Block)+1]
...
for x in nTrials_vector:
    ...
    if  (x % 2) == 0:
        rand_jitter = 0
        rand_degrad = degrad_trig[count_trig]    
        count_trig+=1
    # but for control, yes
    else:
...
for thisCalibration_block in calibration_block:
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    Instruction_CalibrationClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "Instruction_Calibration"-------
    while continueRoutine:
        # get current time
        t = Instruction_CalibrationClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=Instruction_CalibrationClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        # Get the pressed key of the subject
    ...
    while continueRoutine:
        ...
        if Fix_cross_calibration.status == STARTED:
            ...
            if tThisFlipGlobal > Fix_cross_calibration.tStartRefresh + baseline_duration_sec-frameTolerance:
                ...
                win.timeOnFlip(Fix_cross_calibration, 'tStopRefresh')  # time at next scr refresh
    ...
    calibration_block.addData('text.started', text.tStartRefresh)
    calibration_block.addData('text.stopped', text.tStopRefresh)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if text_calibration_repeat.status == STARTED:
            ...
            if tThisFlipGlobal > text_calibration_repeat.tStartRefresh + 10-frameTolerance:
                ...
                win.timeOnFlip(text_calibration_repeat, 'tStopRefresh')  # time at next scr refresh
        ...
        if response_calibration_repeat.status == STARTED:
            ...
            if tThisFlipGlobal > response_calibration_repeat.tStartRefresh + 5-frameTolerance:
                ...
                win.timeOnFlip(response_calibration_repeat, 'tStopRefresh')  # time at next scr refresh
    ...
    calibration_block.addData('text_calibration_repeat.started', text_calibration_repeat.tStartRefresh)
    calibration_block.addData('text_calibration_repeat.stopped', text_calibration_repeat.tStopRefresh)
    thisExp.nextEntry()
...
for thisExp_trial in exp_trials:
    ...
    while continueRoutine:
        ...
        if passed_time > ITI_secs: # wait for 2 seconds (ITI) before checking if the threshold is reached
            ...
            if trial_trig:
                if ratio_elecs > ratio_vec_mean + indiv_factor * ratio_vec_std: # if threshold is reached, current routine ends and stimuli are delivered
        ...
        if text_6.status == STARTED:  # only update if drawing
            text_6.setText(trial_trig, log=False)
