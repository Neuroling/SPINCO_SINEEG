...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Gorka\\Desktop\\SiN\\PsychoPy\\SIN_lastrun.py',
...
pp_t3_end = parallel.ParallelPort(address='0x3FE8')
...
instrExperimentClock = core.Clock()
...
for thisTraining in training:
    ...
    while continueRoutine:
        ...
        if pp_t1_end.status == STARTED:
            ...
            if tThisFlipGlobal > pp_t1_end.tStartRefresh + 0.01-frameTolerance:
                ...
                win.timeOnFlip(pp_t1_end, 'tStopRefresh')  # time at next scr refresh
        ...
        if pp_t2_end.status == STARTED:
            ...
            if tThisFlipGlobal > pp_t2_end.tStartRefresh + 0.01-frameTolerance:
                ...
                win.timeOnFlip(pp_t2_end, 'tStopRefresh')  # time at next scr refresh
        ...
        if pp_t3_end.status == STARTED:
            ...
            if tThisFlipGlobal > pp_t3_end.tStartRefresh + 0.01-frameTolerance:
                ...
                win.timeOnFlip(pp_t3_end, 'tStopRefresh')  # time at next scr refresh
    ...
    training.addData('screenAfterAudio.started', screenAfterAudio.tStartRefresh)
    training.addData('screenAfterAudio.stopped', screenAfterAudio.tStopRefresh)
    if pp_t1_start.status == STARTED:
        win.callOnFlip(pp_t1_start.setData, int(0))
    training.addData('pp_t1_start.started', pp_t1_start.tStart)
    training.addData('pp_t1_start.stopped', pp_t1_start.tStop)
    if pp_t1_end.status == STARTED:
        win.callOnFlip(pp_t1_end.setData, int(0))
    training.addData('pp_t1_end.started', pp_t1_end.tStart)
    training.addData('pp_t1_end.stopped', pp_t1_end.tStop)
    if pp_t2_start.status == STARTED:
        win.callOnFlip(pp_t2_start.setData, int(0))
    training.addData('pp_t2_start.started', pp_t2_start.tStart)
    training.addData('pp_t2_start.stopped', pp_t2_start.tStop)
    if pp_t2_end.status == STARTED:
        win.callOnFlip(pp_t2_end.setData, int(0))
    training.addData('pp_t2_end.started', pp_t2_end.tStart)
    training.addData('pp_t2_end.stopped', pp_t2_end.tStop)
    if pp_t3_start.status == STARTED:
        win.callOnFlip(pp_t3_start.setData, int(0))
    training.addData('pp_t3_start.started', pp_t3_start.tStart)
    training.addData('pp_t3_start.stopped', pp_t3_start.tStop)
    if pp_t3_end.status == STARTED:
        win.callOnFlip(pp_t3_end.setData, int(0))
    training.addData('pp_t3_end.started', pp_t3_end.tStart)
    training.addData('pp_t3_end.stopped', pp_t3_end.tStop)
    if pp_start.status == STARTED:
        win.callOnFlip(pp_start.setData, int(0))
    training.addData('pp_start.started', pp_start.tStart)
    training.addData('pp_start.stopped', pp_start.tStop)
    if pp_end.status == STARTED:
        win.callOnFlip(pp_end.setData, int(0))
    training.addData('pp_end.started', pp_end.tStart)
    training.addData('pp_end.stopped', pp_end.tStop)
    ...
    while continueRoutine:
        ...
        if pp_click1.status == STARTED:
            ...
            if tThisFlipGlobal > pp_click1.tStartRefresh + 1.0-frameTolerance:
                ...
                win.timeOnFlip(pp_click1, 'tStopRefresh')  # time at next scr refresh
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    training.addData('blankScreen.started', blankScreen.tStartRefresh)
    training.addData('blankScreen.stopped', blankScreen.tStopRefresh)
    training.addData('call1.started', call1.tStartRefresh)
    training.addData('call1.stopped', call1.tStopRefresh)
    training.addData('call2.started', call2.tStartRefresh)
    training.addData('call2.stopped', call2.tStopRefresh)
    training.addData('call3.started', call3.tStartRefresh)
    training.addData('call3.stopped', call3.tStopRefresh)
    training.addData('call4.started', call4.tStartRefresh)
    training.addData('call4.stopped', call4.tStopRefresh)
    training.addData('colour1.started', colour1.tStartRefresh)
    training.addData('colour1.stopped', colour1.tStopRefresh)
    training.addData('colour2.started', colour2.tStartRefresh)
    training.addData('colour2.stopped', colour2.tStopRefresh)
    training.addData('colour3.started', colour3.tStartRefresh)
    training.addData('colour3.stopped', colour3.tStopRefresh)
    training.addData('colour4.started', colour4.tStartRefresh)
    training.addData('colour4.stopped', colour4.tStopRefresh)
    training.addData('number1.started', number1.tStartRefresh)
    training.addData('number1.stopped', number1.tStopRefresh)
    training.addData('number2.started', number2.tStartRefresh)
    training.addData('number2.stopped', number2.tStopRefresh)
    training.addData('number3.started', number3.tStartRefresh)
    training.addData('number3.stopped', number3.tStopRefresh)
    training.addData('number4.started', number4.tStartRefresh)
    training.addData('number4.stopped', number4.tStopRefresh)
    ...
    training.addData('mouseClickOnCall.started', mouseClickOnCall.tStart)
    training.addData('mouseClickOnCall.stopped', mouseClickOnCall.tStop)
    # store data for training (TrialHandler)
    ...
    training.addData('mouseClickOnColour.started', mouseClickOnColour.tStart)
    training.addData('mouseClickOnColour.stopped', mouseClickOnColour.tStop)
    if pp_click1.status == STARTED:
        win.callOnFlip(pp_click1.setData, int(0))
    training.addData('pp_click1.started', pp_click1.tStart)
    training.addData('pp_click1.stopped', pp_click1.tStop)
    # store data for training (TrialHandler)
    ...
    training.addData('mouseClickOnNumber.started', mouseClickOnNumber.tStart)
    training.addData('mouseClickOnNumber.stopped', mouseClickOnNumber.tStop)
    ...
    #logging.flush()
    training.addData('blankScreenAfterResponse.started', blankScreenAfterResponse.tStartRefresh)
    training.addData('blankScreenAfterResponse.stopped', blankScreenAfterResponse.tStopRefresh)
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
instrExperimentClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "instrExperiment"-------
while continueRoutine:
    # get current time
    t = instrExperimentClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=instrExperimentClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
for thisComponent in instrExperimentComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('instrExpText.started', instrExpText.tStartRefresh)
thisExp.addData('instrExpText.stopped', instrExpText.tStopRefresh)
