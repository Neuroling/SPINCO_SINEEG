...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Becker\\Documents\\Neurofeedback\\Acquisition Material\\3_Revised_Acquisition_Material_RevDesign_9\\PsychoPy\\triggered_experiment_revDesign.py',
...
Digit2_clear = sound.Sound('A', secs=-1, stereo=False, hamming=False,
    name='Digit2_clear')
Digit2_clear.setVolume(1)
Digit3_clear = sound.Sound('A', secs=-1, stereo=False, hamming=False,
    name='Digit3_clear')
...
text_4 = visual.TextStim(win=win, name='text_4',
    text='In Kürze wirst Du wiedere mehrere Wörter hören, 3 Ziffern nacheinander, das ganze wiederholt.\n\nWie vorher, nach einer solchen Dreier-Reihe bitte die entsprechenden 3 Ziffern mit der Tastatur einzeln eintippen. Du musst jedes Mal auf "Enter" drücken, um zu bestätigen.\n\nBeispiel: Du hast “1”,”2”,”3” gehört, dann tippe ein: “1”, gefolgt von “ENTER”, “2” --> ENTER, “3” --> ENTER.\n\nSobald du bereit bist, kannst du auf "Enter" drücken und dieser Teil des Experimentes wird beginnen.\n',
    font='Arial',
...
sound_digit_2_ME = sound.Sound('A', secs=-1, stereo=False, hamming=False,
    name='sound_digit_2_ME')
sound_digit_2_ME.setVolume(1.0)
sound_digit_3_ME = sound.Sound('A', secs=-1, stereo=False, hamming=False,
    name='sound_digit_3_ME')
sound_digit_3_ME.setVolume(1.0)
...
sound_digit2 = sound.Sound('A', secs=-1, stereo=False, hamming=False,
    name='sound_digit2')
sound_digit2.setVolume(1.0)
sound_digit3 = sound.Sound('A', secs=-1, stereo=False, hamming=False,
    name='sound_digit3')
...
for thisPractice_block in practice_block:
    ...
    practice_trials = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('data/practice_trials.xlsx', selection='0'),
    ...
    for thisPractice_trial in practice_trials:
        ...
        Digit2_clear.setSound(digit2, hamming=False)
        Digit2_clear.setVolume(1, log=False)
        Digit3_clear.setSound(digit3, hamming=False)
...
for thisME_trial in ME_trials:
    ...
    sound_digit_2_ME.setSound(digit2_ME, hamming=False)
    sound_digit_2_ME.setVolume(1.0, log=False)
    sound_digit_3_ME.setSound(digit3_ME, hamming=False)
    sound_digit_3_ME.setVolume(1.0, log=False)
    text_2.setText(ME_trials.thisTrialN)
    # keep track of which components have finished
    Playing_Digits_MEComponents = [sound_digit_1_ME, sound_digit_2_ME, sound_digit_3_ME, text_2]
    ...
    while continueRoutine:
        ...
        # start/stop sound_digit_2_ME
        if sound_digit_2_ME.status == NOT_STARTED and sound_digit_1_ME.status== FINISHED:
            # keep track of start time/frame for later
            sound_digit_2_ME.frameNStart = frameN  # exact frame index
            sound_digit_2_ME.tStart = t  # local t and not account for scr refresh
            sound_digit_2_ME.tStartRefresh = tThisFlipGlobal  # on global time
            sound_digit_2_ME.play()  # start the sound (it finishes automatically)
        # start/stop sound_digit_3_ME
        if sound_digit_3_ME.status == NOT_STARTED and sound_digit_2_ME.status== FINISHED:
            # keep track of start time/frame for later
            sound_digit_3_ME.frameNStart = frameN  # exact frame index
            sound_digit_3_ME.tStart = t  # local t and not account for scr refresh
            sound_digit_3_ME.tStartRefresh = tThisFlipGlobal  # on global time
            sound_digit_3_ME.play()  # start the sound (it finishes automatically)
    ...
    sound_digit_2_ME.stop()  # ensure sound has stopped at end of routine
    ME_trials.addData('sound_digit_2_ME.started', sound_digit_2_ME.tStart)
    ME_trials.addData('sound_digit_2_ME.stopped', sound_digit_2_ME.tStop)
    sound_digit_3_ME.stop()  # ensure sound has stopped at end of routine
    ME_trials.addData('sound_digit_3_ME.started', sound_digit_3_ME.tStart)
    ME_trials.addData('sound_digit_3_ME.stopped', sound_digit_3_ME.tStop)
    ...
    # set up handler to look after randomisation of conditions etc
    response_ME = data.TrialHandler(nReps=nDigits, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
    ...
    # completed nDigits repeats of 'response_ME'
    ...
    text_5.setText(currentTrlCorrect)
    text_8.setText(metric)
    # keep track of which components have finished
    Classify_MEComponents = [text_5, text_8]
    ...
    for thisComponent in Classify_MEComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    ME_trials.addData('text_5.started', text_5.tStartRefresh)
    ME_trials.addData('text_5.stopped', text_5.tStopRefresh)
    ME_trials.addData('text_8.started', text_8.tStartRefresh)
    ME_trials.addData('text_8.stopped', text_8.tStopRefresh)
...
for thisExp_trial in exp_trials:
    ...
    sound_digit2.setSound(digit2, hamming=False)
    sound_digit2.setVolume(1.0, log=False)
    sound_digit3.setSound(digit3, hamming=False)
