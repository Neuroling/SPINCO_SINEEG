...
#### subfolders
```{r,max.height='300px'}
  dirinput <- c("C:/Users/gfraga/gfraga_tmp_neulin/EEG_SIN/Pilot (October 2021)_GFG")
  # Print folder tree    
  fs::dir_tree(dirinput,recurse=TRUE,type='directory')

```
#### subfolders & files
```{r, max.height = '300px' }
fs::dir_tree(dirinput,recurse=TRUE)
```
...
dirinput <- c("C:/Users/gfraga/gfraga_tmp_neulin/EEG_SIN/Pilot (October 2021)_GFG")
  ...
  # Read all files
  files <- gsub(".*\\EEG_SIN/", "", dir(dirinput,recursive =TRUE))  # trimmed list of subdirectories and files from main data directory 
  ...
  for (m in 1:length(measurements)){
    ...
    datSummary[[m]] <-  as.data.frame(cbind(paste0('M',idxs[m]),
      ...
      # Read EEG data (if available) to check dimensions
           
             # function to read and check dims  
            check_eeg_dims <- function(headerfile) {
                        if (file.info(headerfile)$size!=0) {
                          ...
                          dimensions<<- t(as.data.frame(c(0,0)))
                        }  
                }
              
      
          #Apply only to processed.eeg files   
          if (length(which(grepl(pattern = "*processed.vhdr$",x = curfiles)))==1) {
              processedfile <- paste0(dirinput,'/',curfiles[grep(curfiles,pattern='*processed.vhdr')]) 
              info <- suppressWarnings(t(c(check_eeg_dims(processedfile))))  #run func to gather dimensions
              rm(processedfile)
          } else { info <<- t(as.data.frame(c(NA,NA))) }
           
        # Add to main summary table            
        datSummary[[m]] <- cbind(datSummary[[m]], info)
  }

  datSummary <- data.table::rbindlist(datSummary,fill = TRUE)
  colnames(datSummary)  <- c('measurement','total_n_files','eeg_raw','eeg_processed','wav_files?','readme_files?', 'proc.eeg_rows','proc.eeg_cols')
