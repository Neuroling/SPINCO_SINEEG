...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Becker\\Documents\\Neurofeedback\\Acquisition Material\\3_Revised_Acquisition_Material_RevDesign_2\\Acquisition material\\PsychoPy\\triggered_experiment_revDesign_lastrun.py',
...
labels_colomn = np.array(["TrialID","NFB_digit1","NFB_digit2","NFB_digit3","Speaker_trial","Degrad_trial","Triggered_trial","Jitter_trial"]) # define the labels of the columns for excel
...
jitter_vec = np.zeros((len(nTrials_vector),))
jitter_vec[1::2] = np.random.uniform(-1,0,1) * refresh_rate
...
for x in nTrials_vector:
    ...
    # but for control, yes
        ...
        rand_jitter= np.random.uniform(-1,0,1) * refresh_rate
...
exp_trials = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('data/Stimuli_exp_cond.xlsx', selection=trial_nr),
...
for thisExp_trial in exp_trials:
    ...
    # keep track of which components have finished
    signal_checkComponents = [Fixation, text_6, text_2, text_7, text_8]
    ...
    while continueRoutine:
        ...
        if text_6.status == STARTED:  # only update if drawing
            text_6.setText(ratio_elecs, log=False)
        ...
        if text_2.status == STARTED:  # only update if drawing
            text_2.setText(indiv_factor, log=False)
        ...
        if text_7.status == STARTED:  # only update if drawing
            text_7.setText(ratio_vec_mean, log=False)
        ...
        if text_8.status == STARTED:  # only update if drawing
            text_8.setText(ratio_vec_std, log=False)
    ...
    exp_trials.addData('text_2.started', text_2.tStartRefresh)
    exp_trials.addData('text_2.stopped', text_2.tStopRefresh)
    exp_trials.addData('text_7.started', text_7.tStartRefresh)
    exp_trials.addData('text_7.stopped', text_7.tStopRefresh)
    exp_trials.addData('text_8.started', text_8.tStartRefresh)
    exp_trials.addData('text_8.stopped', text_8.tStopRefresh)
