...
This script does not produce any output. It is simply for getting this to work
and then copying the code into Scripts/Gen_stimuli/Gen_speech_noise_sequences/Generate_spreadsheet_Exp2.py
...
n_allocations = n_targets*2 # How many unique & non-overlapping blocks we want
...
for i in range(n_targets):
    indexes += [i * n_targets + item for item in item_allocation_list[:n_targets//2]]
...
for y in allocation_list: # for every list we want...
    ...
    designation_lists[y+n_targets] = [item for item in tmp_list if item not in designation_lists[y]]
